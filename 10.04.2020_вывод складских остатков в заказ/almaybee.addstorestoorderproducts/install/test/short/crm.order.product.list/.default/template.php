...
foreach($arResult['PRODUCTS'] as $product)
{
    ...
    if($arResult['MY_CUSTOM_STORES'] && isset($product['CUSTOM_STORES']) && $product['CUSTOM_STORES']):
        ...
        foreach ($product['CUSTOM_STORES'] as $store) //CRM_ORDER_CUSTOM_STORE_AVAILABLE
        {
            if(in_array($store['STORE_ID'],array_keys($arResult['MY_CUSTOM_STORES'])))
                $storeInfo .= '<div class="crm-order-product-control-amount-desc">'.Loc::getMessage('CRM_ORDER_CUSTOM_STORE_AVAILABLE').' '.$arResult['MY_CUSTOM_STORES'][$store['STORE_ID']]['TITLE'].' ('.$arResult['MY_CUSTOM_STORES'][$store['STORE_ID']]['ADDRESS'].')'.' - '.$store['AMOUNT'].' '.$product['MEASURE_TEXT'].'</div> ';
        }
	...
	';
        ...
        ////		     <div class="crm-order-product-control-amount-desc">'.Loc::getMessage('CRM_ORDER_CUSTOM_STORE_AVAILABLE').': '.(float)$product['AVAILABLE'].' '.$product['MEASURE_TEXT'].'</div>
        ////		     <div class="crm-order-product-control-amount-desc">Стклад 2</div>
